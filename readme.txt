...
结论：最终迁移学习这边用什么框架，大概参数用什么？svm 和 dann 结合？

8) multi branch 多分支任务
    todo
9）模型集成
    已完成【引入更多的分类器，最终结果融合的方式】明天中午
    周二中午 - 周三中午
    周三下午

已完成：
1）参数信息特征，去除hpI 和 mre 两个维度
2）引入更多的分类器，最终结果融合
3）cdan 参数调整 loss 收敛
4）cdan中引入平方对抗函数 和 正则化项
5）早停策略优化：epoch 训练五折后，再启动早停，保存最优模型（todo）【12 or 13 的】
6）
...
逻辑：
    ...
    「创新点：」
            级联
            1）提出一个框架xxxNet（机器学习 + 深度学习）
            2）数据处理：
               数据处理的手段，数据增强的手段，参数信息的引入
            3) 机器学习，多分类器集成，加权决策 （-> 堆叠）
            4）深度学习：
                cadn 下的 mcc 对齐损失loss
                基于损失反馈的自适应调整策略
...
参数器筛选, 随机种子的确定，beta的调整，分类器和cadn的融合机制
早停策略；
