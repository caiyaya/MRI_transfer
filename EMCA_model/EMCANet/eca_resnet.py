...
def conv3x3(in_planes, out_planes, stride=1,modality=19):
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, groups=modality, bias=False)
...
class CifarECABasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, k_size=3, modality=19):  # (self, inplanes, planes,
        ...
        self.conv1 = conv3x3(inplanes, planes, stride, modality)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, 1, modality)
        self.bn2 = nn.BatchNorm2d(planes)
        self.eca = eca_layer(planes, k_size, modality)
        if inplanes != planes:
            self.downsample = nn.Sequential(nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, groups=modality, bias=False),
...
class CifarECAResNet(nn.Module):
    def __init__(self, block, n_size, num_classes=32, k_size=[2, 2, 2], modality=19):
        super(CifarECAResNet, self).__init__()
        self.inplane = modality*2
        self.conv1 = nn.Conv2d(modality, self.inplane, kernel_size=3, stride=1, padding=1, groups=modality, bias=False)
        ...
        self.fc = nn.Linear(modality*8, num_classes)
        self.initialize()
