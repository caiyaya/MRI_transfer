...
for i in range(times):
    ...
    train_dataset = MRIORCT(config, train = True,classify = False,MRI = True)#MRI = False -- CT   
    test_dataset = MRIORCT(config, train = False,classify = False,MRI = True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, **kwargs)

    #res_model = resnet.resnet20(8,num_features = 64, num_classes = config.classes)
    res_model = se_resnet20(num_classes=64, in_channel = 6,reduction=16)
    ...
    plot_embeddings(train_embeddings_cl, train_labels_cl, classes=["NotMVI","MVI"], save_tag = 'MRtrain11')
    val_embeddings_cl, val_labels_cl = extract_embeddings(test_loader, res_model)
    plot_embeddings(val_embeddings_cl, val_labels_cl, classes=["NotMVI","MVI"], save_tag = 'MRtest11')
    ...
    flag=True
    train_dataset = MRIORCT(config, train = True,classify = True,MRI = True)   
    test_dataset = MRIORCT(config, train = False,classify = True,MRI = True)
    ...
    """
    scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer1, milestones=[5,10,15,35])#[25,40,50]--70 [20,35,50,100]
    ...
    torch.save(model1.state_dict(),'./MRbranch.pt')
avgtrain1=0
avgval1=0
for s in avgtrain:
    avgtrain1+=int(s)
...
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, **kwargs)
train_embeddings_cl, train_labels_cl = extract_embeddings(train_loader, res_model)
plot_embeddings(train_embeddings_cl, train_labels_cl, classes=["NotMVI","MVI"], save_tag = 'MRtrain')
val_embeddings_cl, val_labels_cl = extract_embeddings(test_loader, res_model)
plot_embeddings(val_embeddings_cl, val_labels_cl, classes=["NotMVI","MVI"], save_tag = 'MRtest')
